-- 데이터베이스 트리거 : insert, update, delete 할 때 자동으로 동작하는 프로시저
-- 		특정 테이블에 속해있는 객체
...
BEGIN 
 	IF to_char(sysdate, 'HH24:MI') BETWEEN '13:00' AND '15:00' THEN 
 		raise_application_error(-20000,'지금 오후 1시~3시는 작업할수 없습니다.');
 	END IF;
END;
-- 트리거 동작 테스트
...
BEGIN 
	-- 구매 취소한(tbl_buy 테이블에서 삭제)한 데이터 tbl_temp 임시테이블에 insert	: 여러행에 대한 작업(행 트리거)
